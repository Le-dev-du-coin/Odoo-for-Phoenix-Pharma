...
class ResPartner(models.Model):
    ...
    limit_1 = fields.Monetary(
        string="Limite > 500 000 F",
        currency_field="currency_id",
        default=500000,
        tracking=True,
    )
    limit_2 = fields.Monetary(
        string="Limite > 1 000 000 F", currency_field="currency_id", default=1000000
    )
    limit_3 = fields.Monetary(
        string="Limite > 2 000 000 F", currency_field="currency_id", default=2000000
    )
    limit_4 = fields.Monetary(
        string="Limite > 4 000 000 F", currency_field="currency_id", default=4000000
    )
    ...
    def calculate_escompte(self):
        ...
        report = self.env["escompte.ristourne.report"].search(
            [
                ("month", "=", first_day_of_month),
                ("pharmacie_id", "=", partner.company_id.id),
            ],
            limit=1,
        )
